...
class Analysis_model extends CI_Model{
    ...
    protected $_table = "needs_analysis";
    ...
    public function getPrintInfo($id) {
    	if(!empty($id)){
            $data = $this->db->select("id, first_name, sur_name, title, product_name, description, report, company_name,company_logo, company_website, textbox1, textbox2,textbox3,textbox4, color, created_at ")->get_where($this->_table, ['id' => $id])->row_array();
        }else{
            $data = $this->db->select("id, first_name, sur_name, title, product_name, description, report, company_name, company_logo, company_website, textbox1, textbox2,textbox3,textbox4, color, created_at ")->get($this->_table)->result();
        }
